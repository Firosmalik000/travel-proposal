...
Route::middleware(['auth', 'verified'])->group(function () {     /* Get user menus (for sidebar) */ Route::get('api/user-menus', [MenuController::class, 'getUserMenus'])->name('user.menus');
    $registerAdminPortalRoutes = function (string $prefix, bool $withNames = true): void {
        ...
        Route::prefix($prefix.'/website-management')->group(function () use ($nameRoute) {
            ...
            $nameRoute(Route::get('gallery', [GalleryController::class, 'index']), 'gallery.index');
            $nameRoute(Route::post('gallery', [GalleryController::class, 'store']), 'gallery.store');
            $nameRoute(Route::patch('gallery/{galleryItem}', [GalleryController::class, 'update']), 'gallery.update');
            $nameRoute(Route::delete('gallery/{galleryItem}', [GalleryController::class, 'destroy']), 'gallery.destroy');
