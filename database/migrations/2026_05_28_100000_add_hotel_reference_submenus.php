...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($entries as $entry) {
            $exists = collect($children)->contains(fn (mixed $child): bool => is_array($child) && ($child['menu_key'] ?? null) === $entry['menu_key']);
    ...
    public function down(): void
    {
        ...
        $removeKeys = ['hotel_country', 'hotel_city', 'hotel_room_type'];

        $children = collect($children)
            ->reject(fn (mixed $child): bool => is_array($child) && in_array(($child['menu_key'] ?? null), $removeKeys, true))
            ->values()
            ->all();
