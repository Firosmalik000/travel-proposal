            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6">
                <div className="grid gap-6 md:grid-cols-3">
                    {jobs.map((item: Record<string, unknown>) => (
                        <div key={`${item.sort_order}_${localize(item.title, locale)}`} className="rounded-2xl border border-border bg-card/90 p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                            <h3 className="public-heading text-lg font-semibold text-foreground">{localize(item.title, locale)}</h3>
                            <p className="mt-2 text-sm text-muted-foreground">{localize(item.description, locale)}</p>
                            <p className="mt-3 text-xs uppercase tracking-[0.2em] text-primary">{String(item.location || '-')} • {String(item.employment_type || '-')}</p>
                            <p className="mt-3 text-sm text-muted-foreground">{localize(item.requirements, locale)}</p>
                            <button className="mt-4 rounded-full border border-border px-4 py-2 text-xs font-semibold text-foreground transition hover:bg-muted">
                                {localize(page?.content?.cta, locale, 'Lihat Detail')}
                            </button>
                        </div>
                    ))}
                </div>
