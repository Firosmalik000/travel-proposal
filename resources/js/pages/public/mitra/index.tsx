...
export default function Mitra() {
    ...
    const partners = Array.isArray(publicData.partners)
        ? publicData.partners
        : [];
    ...
    return (
        <PublicLayout>
            ...
            <MotionGroup className="mx-auto grid w-full max-w-6xl gap-6 px-4 pb-10 sm:px-6 md:grid-cols-2 lg:grid-cols-3">
                {partners.map((partner: Record<string, unknown>) => (
                    <MotionCard
                        key={String(partner.name)}
                        className="rounded-2xl border border-border bg-card/90 p-6 shadow-sm"
                    >
                        <img
                            src={String(
                                partner.logo_path || '/images/dummy.jpg',
                            )}
                        ...
                        <p className="mt-1 text-xs tracking-[0.2em] text-primary uppercase">
                            {String(partner.category || '-')}
                        </p>
                        <p className="mt-3 text-sm text-muted-foreground">
                            {localize(partner.description, locale)}
                        </p>
                    </MotionCard>
                ))}
            </MotionGroup>
            ...
            <MotionSection className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6">
                <MotionCard className="rounded-3xl border border-border bg-card/90 p-6 text-sm text-muted-foreground shadow-sm">
