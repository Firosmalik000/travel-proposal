...
export default function Legalitas() {
    ...
    const legalDocuments = Array.isArray(publicData.legal_documents) ? publicData.legal_documents : [];
    const bankLines = Array.isArray(page?.content?.bank_lines) ? page.content.bank_lines : [];

    return (
        <PublicLayout>
            <Head title={localize(page?.title, locale, 'Legalitas')}>
                <meta name="description" content={localize(page?.excerpt, locale, 'Informasi legalitas travel')} />
            </Head>
            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-10 pt-6 sm:px-6">
                <div className="rounded-3xl border border-border bg-card/90 px-6 py-8 shadow-lg">
                    ...
                    <h1 className="public-heading mt-4 text-2xl font-semibold text-foreground sm:text-3xl md:text-4xl">
                        {localize(page?.content?.hero?.title, locale, 'Legalitas & Perizinan')}
                    </h1>
                    <p className="mt-2 text-muted-foreground">{localize(page?.content?.hero?.description, locale, '')}</p>
                </div>
            </section>
            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6">
                <div className="grid gap-6 md:grid-cols-2">
                    {legalDocuments.map((item: Record<string, unknown>) => (
                        <div key={`${item.document_number}`} className="rounded-2xl border border-border bg-card/90 p-6 shadow-sm">
                            <h3 className="public-heading text-lg font-semibold text-foreground">{localize(item.title, locale)}</h3>
            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6">
                <h2 className="public-heading text-xl font-semibold text-foreground sm:text-2xl">
                    {localize(page?.content?.docs_title, locale, 'Dokumen Legalitas')}
                </h2>
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                    {legalDocuments.map((item: Record<string, unknown>) => (
                        <div key={`doc_${item.document_number}`} className="flex h-36 items-center justify-center rounded-2xl border border-dashed border-border bg-muted/40 px-4 text-center text-sm font-semibold text-muted-foreground">
            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6">
                <h2 className="public-heading text-xl font-semibold text-foreground sm:text-2xl">
                    {localize(page?.content?.bank_title, locale, 'Rekening Resmi')}
                </h2>
            ...
            <section className="mx-auto w-full max-w-6xl px-4 pb-16 sm:px-6">
                <h2 className="public-heading text-xl font-semibold text-foreground sm:text-2xl">
                    {localize(page?.content?.disclaimer_title, locale, 'Disclaimer')}
                </h2>
