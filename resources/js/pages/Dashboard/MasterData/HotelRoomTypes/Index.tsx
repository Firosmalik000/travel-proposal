...
type RoomType = { id: number; name: string; is_active: boolean };
type Props = {
...
const toForm = (item: RoomType | null): FormData => ({
    name: item?.name ?? '',
    is_active: item?.is_active ?? true,
});

export default function HotelRoomTypesIndex({
    roomTypes,
    filters,
    stats,
}: Props) {
    const { can } = usePermission('hotel_room_type');
    const canCreate = can('create');
    const canEdit = can('edit');
    const canDelete = can('delete');
    const [search, setSearch] = useState(filters.search);
    const [editingItem, setEditingItem] = useState<RoomType | 'new' | null>(
        null,
    );
    ...
    function submit(event: React.FormEvent): void {
        ...
        if (editingItem === 'new') {
            router.post('/admin/master-data/hotel-room-types', form.data, {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('Room type berhasil ditambahkan.');
                    setEditingItem(null);
                },
            });
            return;
        ...
        if (editingItem) {
            router.put(
                `/admin/master-data/hotel-room-types/${editingItem.id}`,
                form.data,
                {
                    preserveScroll: true,
                    onSuccess: () => {
                        toast.success('Room type berhasil diperbarui.');
                        setEditingItem(null);
                    },
                },
            );
            ...
            breadcrumbs={[
                {
                    ...
                    href: '/admin/master-data/hotel-room-types',
                },
            ]}
        >
            <Head title="Master Room Type Hotel" />
                    ...
                    <div className="flex justify-end gap-2">
                        <Button
                            ...
                            onClick={() => {
                                setSearch('');
                                router.get(
                                    '/admin/master-data/hotel-room-types',
                                    {},
                                    {
                                        preserveState: true,
                                        preserveScroll: true,
                                                                ...
                                                                <DropdownMenuItem
                                                                    ...
                                                                    onClick={() => {
                                                                        if (
                                                                            ...
                                                                            router.delete(
                                                                                `/admin/master-data/hotel-room-types/${item.id}`,
                                                                                {
                                                                                    preserveScroll: true,
                                                                                    onSuccess:
                                                                                        () =>
                                                                                            toast.success(
                                                                                                'Room type berhasil dihapus.',
                                                                                            ),
                                                                                },
                                                                            );
                                                                        }
