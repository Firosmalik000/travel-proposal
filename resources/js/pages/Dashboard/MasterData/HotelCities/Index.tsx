...
type FormData = { country_id: string; name: string; is_active: boolean };

const toForm = (item: City | null): FormData => ({
    country_id: item?.country_id ? String(item.country_id) : '',
    name: item?.name ?? '',
    is_active: item?.is_active ?? true,
});

export default function HotelCitiesIndex({
    cities,
    filters,
    stats,
    countryOptions,
}: Props) {
    const { can } = usePermission('hotel_city');
    const canCreate = can('create');
    const canEdit = can('edit');
    const canDelete = can('delete');
    const [search, setSearch] = useState(filters.search);
    const [editingItem, setEditingItem] = useState<City | 'new' | null>(null);
    const form = useForm<FormData>(toForm(null));

    function submit(event: React.FormEvent): void {
        event.preventDefault();
        if (editingItem === 'new') {
            router.post('/admin/master-data/hotel-cities', form.data, {
                preserveScroll: true,
                onSuccess: () => {
        ...
        if (editingItem) {
            router.put(
                `/admin/master-data/hotel-cities/${editingItem.id}`,
                form.data,
                {
                    preserveScroll: true,
                    onSuccess: () => {
    ...
    return (
        <AppSidebarLayout
            breadcrumbs={[
                {
                    label: 'Master Kota',
                    href: '/admin/master-data/hotel-cities',
                },
            ]}
        >
            <Head title="Master Kota Hotel" />
                ...
                <div className="space-y-4 rounded-2xl border border-border/60 bg-card p-4 shadow-sm">
                    <div className="relative">
                        ...
                        <Input
                            ...
                            placeholder="Cari nama kota atau negara..."
                            className="pl-10"
                        />
                    </div>
                    <div className="flex justify-end gap-2">
                        <Button
                            ...
                            onClick={() => {
                                setSearch('');
                                router.get(
                                    '/admin/master-data/hotel-cities',
                                    {},
                                    {
                                        preserveState: true,
                                        preserveScroll: true,
                                                                ...
                                                                <DropdownMenuItem
                                                                    ...
                                                                    onClick={() => {
                                                                        if (
                                                                            ...
                                                                            router.delete(
                                                                                `/admin/master-data/hotel-cities/${item.id}`,
                                                                                {
                                                                                    preserveScroll: true,
                                                                                    onSuccess:
                                                                                        () =>
                ...
                >
                    ...
                    <form onSubmit={submit} className="mt-6 space-y-5">
                        <div className="space-y-4 rounded-2xl border border-border bg-card p-5 shadow-sm">
                            <div>
                                <Label className="mb-1.5 block">Negara</Label>
                                <Select
