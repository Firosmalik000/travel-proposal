...
import { Calendar, MapPin, Clock, Package, Pencil, Star, Trash2, Zap } from 'lucide-react';
...
const typeConfig: Record<string, { label: string; color: string; dot: string }> = {
    reguler: { label: 'Reguler', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300', dot: 'bg-blue-500' },
    hemat: { label: 'Hemat', color: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300', dot: 'bg-green-500' },
    vip: { label: 'VIP', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300', dot: 'bg-amber-500' },
    premium: { label: 'Premium', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300', dot: 'bg-orange-500' },
    private: { label: 'Private', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300', dot: 'bg-purple-500' },
};
                ...
                <div className="relative hidden w-36 shrink-0 overflow-hidden sm:block lg:w-44">
                    ...
                    {!pkg.is_active && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/40">
                            <span className="rounded-full bg-black/60 px-2 py-0.5 text-xs font-medium text-white">Nonaktif</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-between p-4">
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            <span className="ml-auto font-mono text-xs text-muted-foreground">{pkg.code}</span>
                        </div>

                        {/* Name */}
                        <h3 className="mt-1.5 truncate text-base font-bold text-foreground">{name}</h3>

                        {/* Meta pills */}
                        <div className="mt-2 flex flex-wrap gap-2">
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                                <MapPin className="h-3 w-3" /> {pkg.departure_city}
                            </span>
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                                <Clock className="h-3 w-3" /> {pkg.duration_days} hari
                            </span>
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                                <Package className="h-3 w-3" /> {pkg.product_ids.length} produk
                            </span>
                            {pkg.rating_avg && (
                                <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-600 dark:text-amber-400">
                                    <Star className="h-3 w-3 fill-current" /> {pkg.rating_avg}
                                    <span className="font-normal text-muted-foreground">({pkg.rating_count})</span>
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                {pkg.original_price && (
                                    <span className="text-sm text-muted-foreground line-through">
                                        {pkg.original_price.toLocaleString('id-ID')}
                                    </span>
                                )}
                            </div>
                            {nextDeparture ? (
                                <p className="mt-0.5 flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                                    ...
                                    Berangkat {nextDeparture.departure_date} · {nextDeparture.seats_available} seat tersisa
                                </p>
                            ) : (
                                <p className="mt-0.5 text-xs text-muted-foreground">
                                    {activeSchedules === 0 ? 'Belum ada jadwal' : `${activeSchedules} jadwal open`}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <Button size="sm" variant="outline" className="h-8 gap-1.5 text-xs" onClick={() => onEdit(pkg)}>
