import { GalleryItemImage } from '@/components/gallery-item-image';
...
import { AllInPackageCard } from '@/pages/Dashboard/ProductManagement/Packages/AllInPackageCard';
import { PackageOperationalCostCards } from '@/pages/Dashboard/ProductManagement/Packages/PackageOperationalCostCards';
import { PackageSpecificProductManager } from '@/pages/Dashboard/ProductManagement/Packages/PackageSpecificProductManager';
import { ProductSelector } from '@/pages/Dashboard/ProductManagement/Packages/ProductSelector';
import {
    calculateOperationalCostTotals,
    normalizePackageOperationalCosts,
} from '@/pages/Dashboard/ProductManagement/Packages/package-operational-costs';
import type {
    ActivityOption,
    CurrencyOption,
    HotelCityOption,
...
import { packageImageMimeTypes } from '@/pages/Dashboard/ProductManagement/Packages/types';
...
function formatCurrencyRateInput(value: unknown): number | '' {
...
function buildFormData(pkg: Package | null): PackageFormData {
    ...
    const originalPrice = pkg
        ? Number(pkg.original_price ?? pkg.price) > 0
            ? Number(pkg.original_price ?? pkg.price)
            : ''
        : '';
    ...
    const normalizedContent = normalizePackageContent(pkg?.content ?? {});

    if (pkg && normalizedContent.room_original_prices?.dbl == null) {
        normalizedContent.room_original_prices = {
            ...normalizedContent.room_original_prices,
            dbl: Number(pkg.original_price ?? pkg.price) || null,
        };
    }

    if (pkg && normalizedContent.room_prices?.dbl == null) {
        normalizedContent.room_prices = {
            ...normalizedContent.room_prices,
            dbl: Number(pkg.price) || null,
        };
    }
...
function galleryPositionsForSubmission(
    positions: Record<string, PackageImagePosition>,
    existingImages: string[],
    previewUrls: string[],
): Record<string, PackageImagePosition> {
    const normalizedPositions = normalizePackageImagePositions(positions);
    const submittedPositions: Record<string, PackageImagePosition> = {};

    existingImages.forEach((imagePath) => {
        if (normalizedPositions[imagePath]) {
            submittedPositions[imagePath] = normalizedPositions[imagePath];
        }
    });

    previewUrls.forEach((previewUrl, index) => {
        if (normalizedPositions[previewUrl]) {
            submittedPositions[`__new_image_${index}`] =
                normalizedPositions[previewUrl];
        }
    });
...
function addFocToManualEstimateRoomAllocation(
    allocation: Record<keyof typeof estimateRoomCapacities, number>,
    customerCount: number,
    focCount: number,
    availableRoomTypes: Array<keyof typeof estimateRoomCapacities>,
    ...
    const allocatedCapacity = (
        ...
        (total, [roomType, roomCount]) =>
            total + roomCount * estimateRoomCapacities[roomType],
        0,
    );
    ...
    return (
        ...
        (nextAllocation, roomType) => ({
            ...nextAllocation,
            [roomType]: allocation[roomType] + (focAllocation[roomType] ?? 0),
        }),
        { ...allocation },
    );
}
...
function deriveCustomerPriceComposition(
    ...
    const composition = { dbl: 0, trpl: 0, quad: 0 };
    let remainingCustomers = Math.max(0, customerCount);

    if (!allocation) {
        composition.dbl = remainingCustomers;
...
export function PackageEditorWorkspace({
    ...
    const initialExistingImages = shouldResumeCreateDraft
        ...
        : (pkg?.images ?? []);
    const initialGalleryImagePositions = normalizePackageImagePositions(
        initialFormData.content.gallery_positions,
    );
    ...
    const saveDraft = useCallback(
        (payload: PackageDraftPayload): Promise<boolean> => {
            ...
            const queuedRequest = draftSaveQueueRef.current
                ...
                .then(async () => {
                    try {
                        const response = await fetchWithCsrf(draftUrl, {
                            method: 'PUT',
    ...
    async function appendSelectedImages(selectedFiles: File[]) {
        ...
        if (!isHppEditor) {
            ...
            try {
                const response = await fetchWithCsrf(`${draftUrl}/images`, {
                    method: 'POST',
                    credentials: 'same-origin',
                    headers: {
                        Accept: 'application/json',
    ...
    async function submit(event: React.FormEvent) {
        ...
        const submitUrl = isHppEditor
            ...
            : isEdit
              ...
              : packages.store().url;
        const submittedContent = form.data.content as Record<string, unknown>;
        const submittedRoomOriginalPrices =
            typeof submittedContent.room_original_prices === 'object' &&
            submittedContent.room_original_prices !== null
                ? (submittedContent.room_original_prices as Record<
                      string,
                      number | null
                  >)
                : {};
        const basePrice =
            Number(submittedRoomOriginalPrices.dbl) ||
            Number(form.data.original_price) ||
            Number(form.data.price) ||
            0;
        ...
        const resolvedContent = normalizePackageContent({
            ...form.data.content,
            gallery_positions: galleryPositionsForSubmission(
        ...
        try {
            router.post(submitUrl, formData, {
                ...
                onError: (errors) => {
                    draftNavigationBypassRef.current = false;
                    console.error(
                        '[PackageEditorWorkspace] validation errors',
                        {
                            submitUrl,
                            errors,
                        },
            ...
            draftNavigationBypassRef.current = false;
            setIsSubmittingPackage(false);
            console.error('[PackageEditorWorkspace] submit threw', {
                submitUrl,
                error,
            });
    ...
    const roomOriginalPrices =
        ...
        contentObject.room_original_prices !== null
            ? (contentObject.room_original_prices as Record<
              ...
              >)
            : {};
    const fallbackDoubleOriginalPrice =
        Number(form.data.original_price) || Number(form.data.price) || 0;
    const effectiveRoomOriginalPrices = {
        dbl:
            roomOriginalPrices.dbl ??
            inferOriginalRoomPrice(roomPrices.dbl ?? null, discountPercent) ??
            fallbackDoubleOriginalPrice,
        trpl:
    ...
    const basePrice = Number(effectiveRoomOriginalPrices.dbl) || 0;
    const hasDiscount =
        basePrice > 0 &&
        (discountType === 'nominal'
            ? discountNominal > 0
    ...
    const sellingPrice = effectiveRoomSellingPrices.dbl ?? basePrice;
    ...
    const estimatedFocCount = Math.max(
        0,
        Math.floor(Number(operationalCosts.foc.count) || 0),
    );
    ...
    const estimateProductItems = selectedProductsForEstimate
        ...
        .map((product) => {
            ...
            return {
                ...
                focTotalPrice: estimatedFocCount * multiplier * unitPrice,
            };
        });
    ...
    const estimateHotelGroups = selectedProductsForEstimate
        ...
        .map((product) => {
            ...
            const defaultPaidAllocation = quadFirstEstimateRoomAllocation(
                estimatedCustomerCount,
                availableRoomTypes,
            );
            const defaultAllocation = addFocToManualEstimateRoomAllocation(
                defaultPaidAllocation,
                estimatedCustomerCount,
                estimatedFocCount,
                availableRoomTypes,
            );
                    ...
                    [roomType]: Math.max(
                        ...
                        Number(
                            allocationIsManual
                                ...
                                : defaultPaidAllocation[roomType],
                        ),
                    ),
                }),
                {} as Record<keyof typeof estimateRoomCapacities, number>,
            );
            const allocation = allocationIsManual
                ? addFocToManualEstimateRoomAllocation(
                      paidAllocation,
                      estimatedCustomerCount,
                      estimatedFocCount,
                      availableRoomTypes,
            ...
            const rows = (
                ...
                const roomCountLimit = getHotelRoomCountLimit(
                    allocation,
                    roomType,
                    estimatedCustomerCount + estimatedFocCount,
                );
            ...
            const allocationDelta =
                allocatedPax - (estimatedCustomerCount + estimatedFocCount);
            ...
            const paidTotalPrice = (
                ...
                (total, roomType) =>
                    total +
                    paidAllocation[roomType] *
                        multiplier *
                        prices[roomType].unitPrice,
                0,
            );
            ...
            return {
                product,
                allocationIsManual,
                defaultAllocation: defaultPaidAllocation,
                paidAllocation,
                allocation,
                rows,
                allocatedPax,
                ...
                paidTotalPrice,
                focTotalPrice: Math.max(0, totalPrice - paidTotalPrice),
                totalPrice: Math.max(totalPrice, paidTotalPrice),
            };
        });
    ...
    const estimatedFocProductTotal = estimateProductItems.reduce(
        (total, item) => total + item.focTotalPrice,
        0,
    );
    ...
    const estimatedPaidHotelTotal = estimateHotelGroups.reduce(
        (total, group) => total + group.paidTotalPrice,
        0,
    );
    const estimatedFocHotelTotal = estimateHotelGroups.reduce(
        (total, group) => total + group.focTotalPrice,
        0,
    );
    const estimatedCustomers = deriveCustomerPriceComposition(
        estimatedCustomerCount,
        estimateHotelGroups[0]?.paidAllocation,
    );
    ...
    const estimatedAllInTotal = form.data.all_in.enabled
        ...
        : 0;
    const estimatedFocAllInTotal = form.data.all_in.enabled
        ? convertEstimatePriceToIdr(
              form.data.all_in.price_per_pax,
              form.data.all_in.currency,
          ) * estimatedFocCount
    ...
    const estimatedHotelPerPax =
        estimatedCustomerCount > 0
            ? Math.floor(estimatedPaidHotelTotal / estimatedCustomerCount)
            : 0;
    ...
    const estimatedGrandTotal =
        ...
        estimatedFocProductTotal +
        estimatedHotelTotal +
        estimatedAllInTotal +
        estimatedFocAllInTotal +
    ...
    function applyPendingDraft() {
        ...
        setGalleryImagePositions(
            normalizePackageImagePositions(
                restoredData.content.gallery_positions,
            ),
        );
    ...
    async function discardDraft() {
        ...
        try {
            ...
            setGalleryImagePositions(
                normalizePackageImagePositions(
                    resetData.content.gallery_positions,
                ),
            );
            clearLocalImagePreviews();
                    ...
                    <TabsContent value="info" className="mt-4">
                        ...
                        <FieldGroup>
                            ...
                            <PackageOperationalCostCards
                                ...
                                focHotelTotal={estimatedFocHotelTotal}
                                focProductTotal={estimatedFocProductTotal}
                                focAllInTotal={estimatedFocAllInTotal}
                                onChange={(nextCosts) =>
                                    updateHppEstimate({
                                        ...hppEstimate,
                                        ...
                                        <GalleryItemImage
                                            ...
                                            onEdit={(url, position) => {
                                                ...
                                                form.setData('content', {
                                                    ...
                                                    gallery_positions: {
                                                        ...normalizePackageImagePositions(
                                                            contentObject.gallery_positions,
                                                        ),
                                                        [url]: position,
                                                    },
                                                });
                                                toast.success(
                                                    'Posisi foto berhasil diatur',
                                                );
                                            }}
                                            onDelete={() =>
                                                item.kind === 'existing'
                                            ...
                                            savedPosition={
                                                galleryImagePositions[item.url]
                                            }
                                        />
                                    ))}
                            ...
                            <>
                                ...
                                <div className="space-y-4">
                                            ...
                                            {hasOperationalCostConfiguration ? (
                                                <div className="divide-y divide-border/60 text-sm">
                                                    {[
                                                        [
                                                            'FOC',
                                                            estimatedFocHotelTotal +
                                                                estimatedFocProductTotal +
                                                                estimatedFocAllInTotal,
                                                            `${estimatedFocCount} orang, hotel Quad ${formatEstimateCurrency(estimatedFocHotelTotal)}, produk ${formatEstimateCurrency(estimatedFocProductTotal)}, All In ${formatEstimateCurrency(estimatedFocAllInTotal)}`,
                                                        ],
                                                        [
                                                            'SDM',
                                                        ...
                                                        [
                                                            ...
                                                            `${operationalCosts.photographer.count} x ${formatEstimateCurrency(operationalCosts.photographer.daily_salary)} ${operationalCosts.photographer.currency} x ${operationalCosts.photographer.days} hari`,
                                                        ],
                                                        [
                                                            'Tour Leader',
                                                            estimatedOperationalTotals.tourLeader,
                                                            `${operationalCosts.tour_leader.count} x (gaji ${formatEstimateCurrency(operationalCosts.tour_leader.salary_per_trip)} ${operationalCosts.tour_leader.currency} + hotel ${formatEstimateCurrency(operationalCosts.tour_leader.include_hotel ? estimatedHotelPerPax : 0)} + tiket/visa ${formatEstimateCurrency(operationalCosts.tour_leader.include_ticket_and_visa ? estimatedTicketAndVisaPerPax : 0)})`,
                                                        ],
                                                        [
                                                            'Muthawwif',
                                                            estimatedOperationalTotals.muthawwif,
                                    ...
                                    <div className="flex flex-col rounded-2xl border border-border bg-card p-4 sm:p-5">
                                        ...
                                        <div className="order-3 mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-border/60 pt-4 lg:grid-cols-4">
                                            <div>
                                                <p className="text-xs text-muted-foreground">
                                                    HPP / jamaah
                                                </p>
                                        ...
                                        <div className="order-2 mt-5 border-t border-border/60 pt-4">
                                            ...
                                            {estimatedRoomPriceBreakdown.length >
                                            0 ? (
                                        ...
                                        <div className="order-1 grid gap-5 pt-5">
                                            <p className="text-sm font-semibold text-foreground">
                                                Harga dasar dan potongan
                                            </p>
                                            <div className="grid gap-3 lg:grid-cols-2">
                                                <Field label="Tipe Potongan">
                                                    <Select
                                                        value={discountType}
                                                        onValueChange={(
                                                            value:
                                            ...
                                            <div className="grid gap-3 border-t border-border/60 pt-4 md:grid-cols-3">
                                                {(
                                                    [
                                                        ['dbl', 'Double (DBL)'],
                                                        [
                                                    ...
                                                    <Field
                                                        ...
                                                        error={
                                                            errors[
                                                                `content.room_original_prices.${roomType}`
                                                            ]
                                                        }
                                                    >
                                                        <div className="relative">
